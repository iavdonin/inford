...
def parse_url(url):
    ...
    table = soup.find('div', class_='trades-table_wrapper').find('table', class_='simple-little-table trades-table')
...
def parse_description_tikers(tikers):
    ...
    for name in tikers:
        ...
        soup = BeautifulSoup(r.text, 'lxml')
        try:
            description_tikers.append(soup.find('div', class_='SecurityInfoPure__wrapper_35vdA').text)
        except:
